...
\begin{rep}
\begin{enumerate}
\item $\mathcal{H}_0$ est simple et $\mathcal{H}_1$ est composite.
\item $\mathcal{C} = \left\{ (x_1,\dots, x_{20}) \in \{0,1\}^n  :  x_1 + \dots + x_n \in \{ 0,1\} \cup \{7,\dots,20 \}\right \}$
\item 0,1558678
\item $1 - \Pi(p)$
\item Non
\item Région critique devrait dépendre de $n$.
\end{enumerate}
\end{rep}
\begin{sol}
\begin{enumerate}
...
\begin{enumerate}
\item Démo
\item Démo. $C^* = \frac{\lambda_1 - \lambda_0}{\ln{\lambda_1} - \ln{\lambda_0}}$.
\item $c = 0,1$
\item $\alpha = 0,264$ et $\beta = 0,6766764$.
\end{enumerate}
\end{rep}
\begin{sol}
\begin{enumerate}
