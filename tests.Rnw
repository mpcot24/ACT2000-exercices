...
On fournit également les valeurs des fonctions de log-vraisemblance maximisées de même que les estimations des estimateurs du maximum de vraisemblance ajustées avec \verb|fitdistr|:
<<echo=TRUE>>=
library(MASS)
...
\item Quelle est la statistique exhaustive pour $\beta$ dans le cas d'une loi Weibull?
