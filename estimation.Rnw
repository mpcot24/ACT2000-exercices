...
\end{enumerate}
 \begin{rep}
  \begin{enumerate}  
   \item $f_{min}(x) = \alpha n \beta^{\alpha n} x^{-(n \alpha + 1}$
   \item Démo: Définition d'un estimateur convergent.
   \item $B(\hat{\beta}) = \frac{\beta}{\beta n - 1}$ converge vers 0.
   \item Démo par le théorème de factorisation de Fisher-Neyman.
   \item Idem
   \item $\prod_{i = 1}^n X_i$ et $\min(X_1, \dots, X_n)$.
...
\begin{enumerate}
\item Trouver une statistique exhaustive $T$ pour $\beta$.
\item Montrer que $\hat{\beta} = X_1$ est un estimateur sans biais de $\beta$.
\item Sans effectuer de calculs, expliquer qualitativement pourquoi $\hat{\beta}$ ne semble pas être un bon estimateur de $\beta$.
\item Appliquer le théorème de Rao--Blackwell sur $\hat{\beta}$ pour trouver un meilleur estimateur $\tilde{\beta}$.
...
\item $T = \bar{X}_n$
\item Démo
\item Une seule observation pour déterminer le paramètre
\item Démo
\item Oui
\end{enumerate}
\end{rep}
\begin{sol} 
\begin{enumerate}
\item  On applique le théorème de factorisation de Fisher-Neymann en trouvant d'abord la fonction de vraisemblance et en la factorisant ensuite:
\begin{align*}
L(x, \beta) &= \prod_{i = 1}^{n} \frac{1}{\beta} e^{-\frac{1}{\beta} x_i} \\
              &= \beta^{-n} e^{-\beta^{-1} \sum_{i=1}^n x_i} \times 1
\end{align*}
Ainsi, $\hat{\beta} = \sum_{i = 1}^n X_i$ ou $\hat{\beta} = \bar{X}_n$ sont des statistiques exhaustives pour $\beta$ par le théorème de factorisation de Fisher-Neymann.
\item Dans le contexte, $X_1 \sim \mathcal{E}xp(\beta)$. Ainsi,  $\ex[X_1] = \beta$, donc $\hat{\beta}$ est un estimateur sans biais pour $\beta$.
...
\item Comme $\hat{\beta}$ est un estimateur sans biais pour $\beta$ et comme $T = \sum_{i=1}^n X_i$ est exhaustive pour $\beta$, on peut appliquer le théorème de Rao--Blackwell. Ainsi, on obtient que
$$
\tilde{\beta} = \ex[{\hat{\beta} | T}].
$$

Remarque: $ T \sim \mathcal{G}(n, \beta)$ et posons $T^* = \sum_{i = 2}^n X_i \sim \mathcal{G}(n-1, \beta) \, (\ex[{T^*}] = (n-1)\beta \perp X_1)$.
...
&= \frac{\beta^{-1} e^{-\beta^{-1} x_1} \beta^{-(n-1)}(t - x_1)^{n-2} e^{-\beta^{-1} (t - x_1)} \Gamma(n)}{\beta^{-n}t^{n-1} e^{-\beta^{-1} t}\Gamma(n-1)} \\
&= (n-1) \frac{(t - x_1)^{n-2}}{t^{n-1}} \\
&= (n-1) \frac{t^{n-2}(1 - \frac{x_1}{t})^{n-2}}{t^{n-1}} \\
&= (n-1) \frac{(1 - \frac{x_1}{t})^{n-2}}{t}, 0 < x_1 < t.
\end{align*}
...
\begin{align*}
\ex[{\hat{\lambda} | T = t}] &= \int_0^t x_1  (n-1) \frac{(1 - \frac{x_1}{t})^{n-2}}{t} \, dx_1 \\
&= \left[x_1 (1 - \frac{x_1}{t})^{n-1}\right]_0^t - \int_0^t (1 - \frac{x_1}{t})^{n-1} dx_1 \\
&= \frac{t}{n} \\
&= \bar{X}_n.
\end{align*}
...
\item Comme cet estimateur est sans biais et qu'il est une fonction de la statistique exhaustive $T$, $\tilde{\beta}$ est le MVUE de $\ex[X]$. On peut vérifier que sa variance est inférieure à celle de $\hat{\beta}$.
\end{enumerate}
\end{sol}
\end{exercice}
