...
\begin{exercice}
Soit $X_i$ le montant de réclamation pour l'assuré $i$, pour $i=1,\ldots,n$. On suppose que  $X_1, \dots, X_n$ est un échantillon aléatoire tiré d'une loi exponentielle avec paramètre $\lambda$ inconnu telle que $\esp{X} = \frac{1}{\lambda}$. 
\begin{enumerate}
\item Trouver une statistique exhaustive $T$ pour $\lambda$.
\item Montrer que $\hat{\lambda} = X_1$ est un estimateur sans biais de $\theta = \esp{X}$.
\item Sans effectuer de calculs, expliquer qualitativement pourquoi $\hat{\lambda}$ ne semble pas être un bon estimateur de $\lambda$.
\item Appliquer le théorème de Rao--Blackwell sur $\hat{\lambda}$ pour trouver un meilleur estimateur $\tilde{\lambda}$.
\item Est-ce le MVUE? Justifier.
\end{enumerate}
\begin{sol} 
\begin{enumerate}
\item  On applique le théorème de factorisation de Fisher-Neymann en trouvant d'abord la log-vraisemblance et en la factorisant:
\begin{align*}
L(x, \lambda) &= \prod_{i = 1}^{n} \lambda e^{-\lambda x_i} \\
              &= \lambda^n e^{-\lambda \sum_{i=1}^n x_i} \\
l(x, \lambda) &= n \ln{\lambda} - \lambda \sum_{i =1}^n x_i \\
        0 &= \frac{n}{\hat{\lambda}} - \sum_{i =1}^n x_i \\
        \hat{\lambda} &= \bar{X}_n \;\; \text{ou encore} \\
        \hat{\lambda} &= \sum_{i=1}^n 
\end{align*}
sont des statistiques exhaustives pour $\lambda$.
\item Dans le contexte, $X_1 \sim \mathcal{E}xp(\lambda)$. Ainsi,  $\ex[{\hat{\lambda}] = \frac{1}{\lambda}} = \theta$.
\item La valeur de $X_1$ sera la seule observation pour déterminer le seul paramètre du modèle à utiliser. Par exemple, la prime pure d'un groupe d'assurance collective serait uniquement déterminée par un de ses utilisateurs, ce qui peut ne pas représenter adéquatement le comportement du groupe au complet. Un enjeu de compétivité sur le marché pourrait survenir.

\item Comme $\hat{\lambda}$ est un estimateur sans biais pour $\lambda$ et comme $T = \sum_{i=1}^n X_i$ est exhaustive pour $\lambda$, on peut appliquer le théorème de Rao-Blackwell. Ainsi, on obtient que
$$
\tilde{\lambda} = \ex[{\hat{\lambda} | T}].
$$

Remarque: $ T \sim \mathcal{G}(n, \lambda)$ et posons $T* = \sum_{i = 2}^n X_i \sim \mathcal{G}(n-1, \lambda) (\ex[{T*}] = \frac{n-1}{\lambda} \perp X_1)$.

On développe d'abord $f_{X_1 | T} (x | T)$ en appliquant la définition des probabilités conditionnelles 
\begin{align*}
f_{X_1 | T} (x_1 | T) &= \frac{f_{X_1, T} (x_1, t)}{f_T(t)} \\
&= \frac{f_{X_1, T*} (x_1, t - x_1)}{f_T(t)} \\
&= \frac{f_{X_1} (x_1) f_{T*}(t - x_1)}{f_T(t)} \\
&= \frac{\lambda e^{-\lambda x_1} \lambda^{n-1}(t - x_1)^{n-2} e^{-\lambda (t - x_1)} \Gamma(n)}{\lambda^{n}t^{n-1} e^{-\lambda t}\Gamma(n-1)} \\
&= (n-1) \frac{(t - x_1)^{n-2}}{t^{n-1}} \\
&= (n-1) \frac{(1 - \frac{x_1}{s})^{n-2}}{t}, 0 < x_1 < t.
\end{align*}

Ainsi, on peut trouver l'espérance conditionnelle en intégrant par parties en posant $u = x_1$ et d$v = (\frac{n-1}{s} (1 - \frac{x_1}{s})^{n -2}.$ On remarque que le d$v$ de la substitution ici s'intègre magiquement avec une primitive de base.

\begin{align*}
\ex[{\hat{\lambda} | T = t}] &= \int_0^t x_1  (n-1) \frac{(1 - \frac{x_1}{s})^{n-2}}{t} \, dx_1 \\
&= \left[x_1 (1 - \frac{x_1}{s})^{n-1}\right]_0^t - \int_0^t (1 - \frac{x_1}{t})^{(n-1)} dx_1 \\
&= \frac{t}{n} \\
&= \bar{X}_n.
\end{align*}

Ainsi, par le théorème de Rao-Blackwell, on obtient qu'un meilleur estimateur $\tilde{\theta}$ est $\bar{X}_n$. De plus, on remarque que $\ex[{\tilde{\theta}}] = \frac{\ex[{T}]}{n} = \ex[X] = \theta$ est sans biais. 

\item Comme cet estimateur est sans biais et qu'il est une fonction de la statistique exhaustive $T$, $\tilde{\lambda}$ est le MVUE de $\ex[X]$. On peut vérifier que sa variance est inférieure à celle de $\hat{\lambda}$.
\end{enumerate}
\end{sol}
\end{exercice}
