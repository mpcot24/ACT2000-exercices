...
$$
\sigma^2_{TaPaCt} = 32,9 \, \text{pour un échantillon} \, n = 10
$$

Les analyses de dossiers de nouveaux clients chez \emph{AÉACT} font l'objet d'une politique interne selon laquelle on pose trois hypothèses:
\begin{itemize}
...
\item Comme AÉACT a peu de liquidités, elle choisira l'entreprise avec la variance échantillonnale obtenue dans l'analyse qui est la plus basse.
\end{itemize}
...
\item Calculer $\prob{\frac{S^2_{TaPaCt}}{S^2_{Markétact}} < 1}$.
...
\begin{sol}
\begin{enumerate}
\item Il faut également assumer que tous les risques de deux entreprises sont indépendants pour utiliser la loi F. C'est réaliste dans le contexte comme leur domaine de pratique est différent et comme elles sont basées dans des villes différentes.
...
\begin{align*}
\prob{\frac{S^2_{TaPaCt}}{S^2_{Markétact}} < 1} &= \prob{\frac{S^2_{TaPaCt}}{2 S^2_{Markétact}} < 0,5} \\
&= 0,1558435 
\end{align*}

\item Comme il est plus probable que $\frac{S^2_{TaPaCt}}{S^2_{Markétact}} \geq 1$, il est plus probable que $S^2_{TaPaCt} \geq S^2_{Markétact}$. Ainsi, il est plus probable que Markétact soit retenue.
\end{enumerate}
\end{sol}
\end{exercice}
