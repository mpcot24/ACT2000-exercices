...
\begin{rep}
    \begin{enumerate}
		\item Oui. $S^2_n \sim Ga(n-1/2, 2\sigma^2 / (n-1))$ donc $\esp{S^2_n} = \sigma^2$ et $\var{S^2_n} = \frac{2\sigma^4}{n-1}$.
    \end{enumerate}
\end{rep}
\begin{sol}
...
\begin{exercice}
  ...
  \begin{rep}
  	\begin{enumerate}
   	 \item Démo: réutiliser la définition de la loi F
    \end{enumerate}
  \end{rep}
  \begin{sol}
    On a
...
\begin{exercice}
Un assureur de dommages pour entreprises, \emph{AÉACT}, se demande s'il devrait couvrir les risques de \emph{TaPaCt}, une entreprise bien établie de production de tapis avec une usine qui comporte des machines fragiles et dispendieuses ou les risques de \emph{Markétact}, une nouvelle firme de marketing pour laquelle la majorité des employé.e.s sont en télétravail. Les deux entreprises sont basées dans des villes différentes.
...
Les analyses de dossiers de nouveaux clients chez \emph{AÉACT} font l'objet d'une politique interne selon laquelle on pose trois postulats:
\begin{itemize}
\item Les sinistres sont normalement distribués.
\item La variance des dommages des entreprises avec beaucoup de moyens de production est le double de la variance des dommages des entreprises dont la majorité des membres sont en télétravail.
\item Comme \emph{AÉACT} a peu de liquidités, elle choisira l'entreprise avec la variance échantillonnale obtenue dans l'analyse qui est la plus basse.
\end{itemize}

Avant de récolter un échantillon pour Markétact de taille $m = 11$, \emph{AÉACT} a besoin de votre aide.
\begin{enumerate}
\item La prochaine étape de l'analyse implique d'utiliser la loi F. Quelle hypothèse est manquante dans l'énoncé pour utiliser cette loi?

\item Supposons que l'hypothèse manquante en a) est également posée, de quelle façon la loi F peut-elle être utile?

\item Calculer $\prob{S^2_{TaPaCt}/S^2_{Markétact} < 1}$.
\item En amont de l'analyse des projections et en regard du résultat précédent, quelle décision est la plus probable pour AÉACT?
\end{enumerate}
\begin{rep}
...
\begin{sol}
\begin{enumerate}
\item Il faut également supposer que tous les risques de deux entreprises sont indépendants pour utiliser la loi F. C'est réaliste dans le contexte comme leur domaine de pratique est différent et comme elles sont basées dans des villes différentes.

\item La loi F peut être utile afin de comparer les variances échantillonnales des deux entreprises pour les échantillons analysés lors du processus de sélection (les projections). L'assureur pourra faire un choix selon ce résultat pour choisir l'entreprise la moins risquée.
...
\item Comme il est plus probable que $S^2_{TaPaCt}/S^2_{Markétact} \geq 1$ que $S^2_{TaPaCt}/S^2_{Markétact} < 1$, il est plus probable que $S^2_{TaPaCt} \geq S^2_{Markétact}$. Ainsi, il est plus probable que Markétact soit retenue que TaPaCt soit retenue à la fin du processus de sélection.
\end{enumerate}
\end{sol}
\end{exercice}
